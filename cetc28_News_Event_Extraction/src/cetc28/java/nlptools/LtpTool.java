...
//		  SRL.release();
	...
	public static void main(String[] args) throws IOException {
		// TODO Auto-generated method stub
		BufferedWriter bw1 = new BufferedWriter(new FileWriter("src/train_sents_seg.txt"));
		...
		while((sent = br.readLine())!=null)
		{
			...
			List<String> tagList = LtpTool.getPosTag(words);
			System.out.println(tagList);
			String re2 = "";
			for(int i =0; i < tagList.size();i++)
			{
				re2 = re2 + tagList.get(i);
				if(i < tagList.size() - 1)
				{
					re2 += ' ';
				} 
		...
		br.close();
		bw1.close();
		String textString = "奥巴马 就 斯诺登 问题  约谈  普京";
		String[] tokens = textString.split("\\s+");
		System.out.println(tokens);
		List<String> wordList = new ArrayList<>();
		for(String token : tokens){wordList.add(token);}
