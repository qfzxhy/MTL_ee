	...
	public void runCrawler() throws UnsupportedEncodingException, IOException, SQLException, ParseException {
		...
		for(int i = 1; i<results.length; i++) { //results.length
			...
			if(matcherURL.find()) {
				 ...
				 * add by qianf
				 */
//				if(nInsert.getUrl()==null||nInsert.getTitle()==null||nInsert.getTitle().equalsIgnoreCase("")
//						||nInsert.getContent()==null||nInsert.getContent().equalsIgnoreCase("")){
//					System.out.println("empty or no url");
//					return;
//				}
				int newsId = dbtool.selectMaxIdhuanqiu_world()+1;
				if(newsId>=0)
				{
					dbtool.deleteExcessHuanqiu_world("huanqiu_world", newsId);
//					LabelItem extractResult = runDetection.GetEventInforfromNews("huanqiu_world", String.valueOf(newsId), nInsert.getUpdateTime(), nInsert.getTitle());
//					extractResult.Print();
	...
	private void dealItemContent(String s, String url, News n) throws UnsupportedEncodingException, IOException {
		...
		if(contentStart.length>1){
			...
			for(String u: picList){
				...
				String newImageName="huanqiuworld_img/"+u.substring(u.lastIndexOf("/"));
